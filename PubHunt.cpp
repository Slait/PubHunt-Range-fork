...
void PubHunt::workThread(int threadId, const std::string& deviceName) {
    if (threadId < 0 || threadId >= 128) {
        _logger->Log(LogLevel::ERROR1, "ThreadId %d out of bounds for status arrays.", threadId);
        return;
    }
...
void PubHunt::FindKeyGPU(int engineIndex, const std::string& deviceName) {
    ...
    if (engineIndex < 0 || (unsigned int)engineIndex >= _deviceCount || !_gpuEngines[engineIndex]) {
        ...
        try {
            ...
            _logger->Log(LogLevel::ERROR1, "Exception creating GPUEngine: %s", e.what());
        ...
        if (!_gpuEngines[engineIndex] || _gpuEngines[engineIndex]->deviceName.empty()) {
             _logger->Log(LogLevel::ERROR1, "GPUEngine initialization failed for device %s", _deviceNamesList[engineIndex].c_str());
    ...
    if (!currentEngine) {
        _logger->Log(LogLevel::ERROR1, "No GPUEngine for index %d in FindKeyGPU.", engineIndex);
        isAlive[engineIndex] = false;
        hasStarted[engineIndex] = true;
        return;
    }
